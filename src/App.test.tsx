...
describe('Meal Prep',()=>{
 ...
 it('öppnar lokal bildimport från plusmenyn',async()=>{render(<App/>);await userEvent.click(screen.getByRole('button',{name:'Lägg till'}));const button=screen.getByRole('button',{name:/Importera från bilder/});expect(button).toBeEnabled();await userEvent.click(button);expect(screen.getByRole('heading',{name:'Importera recept'})).toBeInTheDocument();expect(screen.getByText('Bearbetas lokalt')).toBeInTheDocument()})
})
