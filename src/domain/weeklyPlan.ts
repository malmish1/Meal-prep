...
export const SCORE_WEIGHTS = { favorite: 22, rating: 2.2, proteinHigh: .35, proteinVeryHigh: .5, mealPrep: 10, campaign: 7, inventory: 9, useFirst: 15, swedish: 6, recentPenalty: 18, calorieDistance: 18 } as const;
...
export function recipeToRecommendation(r: Recipe, category: MealCategory, context: { dailyCalories: number | null; proteinPriority: string; promotions: PromotionItem[]; inventory: InventoryItem[]; macroFilters: MacroFilters }, diagnostics?: FilterDiagnostics): WeeklyRecommendation | null {
  const failure = macroFailure({ calories: r.caloriesPerServing, protein: r.proteinGramsPerServing, carbs: r.carbsGramsPerServing, fat: r.fatGramsPerServing }, context.macroFilters ?? EMPTY_MACRO_FILTERS);
  if (failure) { if (diagnostics) diagnostics[failure]++; return null; }
  if (!mealType(r, category)) { if (diagnostics) diagnostics.mealType++; return null; }
  if (r.sourceType === "external" && !isExternalRecipeEligibleForWeeklyRecommendation(r)) { if (diagnostics) diagnostics.incompleteMacros++; return null; }
  const names = r.ingredients.map(x => x.name.toLocaleLowerCase("sv")), promotionMatches = context.promotions.filter(p => !p.ignored && names.some(n => n.includes(p.normalizedName) || p.normalizedName.includes(n))), inventoryMatches = context.inventory.filter(i => names.some(n => n.includes(i.normalizedName) || i.normalizedName.includes(n))), target = (context.dailyCalories ?? 2500) * (category === "snack" ? .14 : category === "breakfast" ? .2 : .25), distance = Math.min(1, Math.abs((r.caloriesPerServing ?? target) - target) / target);
  let score = (r.favorite ? SCORE_WEIGHTS.favorite : 0) + (r.rating ?? 0) * SCORE_WEIGHTS.rating + (r.proteinGramsPerServing ?? 0) * (context.proteinPriority === "veryHigh" ? SCORE_WEIGHTS.proteinVeryHigh : context.proteinPriority === "high" ? SCORE_WEIGHTS.proteinHigh : .2) + (r.mealPrepSuitability === "excellent" || r.mealPrepSuitability === "very_good" ? SCORE_WEIGHTS.mealPrep : 0) + promotionMatches.length * SCORE_WEIGHTS.campaign + inventoryMatches.length * SCORE_WEIGHTS.inventory + inventoryMatches.filter(x => x.useFirst).length * SCORE_WEIGHTS.useFirst - distance * SCORE_WEIGHTS.calorieDistance;
  if (r.lastCookedAt && Date.now() - new Date(r.lastCookedAt).getTime() < 14 * 86400000) score -= SCORE_WEIGHTS.recentPenalty;
  const reasons = [r.favorite ? "❤️ Ett av dina favoritrecept" : "", inventoryMatches[0] ? `🏠 Du har ${inventoryMatches[0].displayName} hemma` : "", promotionMatches[0] ? `🔥 Matchar ${promotionMatches[0].displayName} på kampanj` : "", `${r.proteinGramsPerServing} g protein`, r.mealPrepSuitability ? "🍱 Bra för matlådor" : ""].filter(Boolean).slice(0, 3);
  return { id: r.id, category, title: r.title, sourceName: r.sourceType === "external" ? r.sourceName : "Mitt recept", sourceUrl: r.sourceUrl, sourceType: r.sourceType === "external" ? "external" : "personal", calories: r.caloriesPerServing!, protein: r.proteinGramsPerServing!, carbs: r.carbsGramsPerServing!, fat: r.fatGramsPerServing!, servings: r.servings ?? null, ingredients: r.ingredients.map(x => ({ name: x.name, quantity: x.quantity, unit: x.unit })), mealPrepSuitability: r.mealPrepSuitability, primaryProtein: r.primaryProtein, cuisine: r.cuisine, favorite: r.favorite, score, reasons };
}
