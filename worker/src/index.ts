...
const recipePrompt = `Interpret all attached screenshots, in their given order, as ONE recipe. Semantically distinguish title, introduction, ingredients, instructions, original recipe yield, nutrition and source. Ignore phone clock, status bar, browser/navigation/social controls, likes, comments, ads and unrelated recommendations. Remove overlap duplicates across screenshots. Detect the recipe language and preserve the useful original recipe text in originalText for internal reference. Return originalServings as the numeric yield only when explicitly supported by phrases such as "4 portioner", "serves 4", "makes 6 servings" or "för 2 personer"; otherwise return null. Never derive yield from ingredient amounts.

Also return servings with exactly the same value as originalServings. This temporary duplicate keeps the currently deployed client compatible while the Worker and GitHub Pages deploy independently.

The normal output fields title, description, ingredient names, ingredient notes, instructions and warnings MUST be natural Swedish. If the source is English, translate contextually rather than literally (for example "fold in" becomes "vänd ner"). Convert imperial/US cooking units into practical Swedish metric cooking units: oz and lb to sensibly rounded g/kg; fl oz to ml/dl; cups to approximately 2.4 dl per cup unless a supported ingredient-specific conversion is explicitly available; tbsp to msk; tsp to tsk; inches to cm; Fahrenheit to sensibly rounded Celsius (400°F ≈ 200°C). Never use a universal cup-to-grams conversion. Prefer g, kg, ml, dl, l, tsk, msk and st, with useful culinary rounding rather than awkward precision. Preserve the meaning and do not invent oven mode or unsupported details.

Never invent missing quantities, ingredients, times, temperatures, servings or nutrition: use null/empty values and a short Swedish warning. Mark an ingredient uncertain when its name, amount or unit cannot be read confidently. Confidence values must be 0–1. Return only the required structured result.`;
