...
import { discoverySchema, likelyIndividualRecipe, validDiscovery } from "./discoverySchema";
...
const discoveryPrompt=`Search the live web for 4-8 real, individual recipe detail pages matching the user's request. Every candidate must be one specific named dish on its own recipe page. Never return a collection, gallery, listicle, topic, category or search page, even when it contains many recipes. Prefer reputable established recipe publishers, supermarkets and food publications, with practical high-protein and meal-prep-friendly options when relevant. Open and verify each individual recipe page, then preserve its publisher and canonical HTTP(S) URL. Never invent a URL, nutrition number, quantity or instruction. Include ingredients/instructions only when available in search evidence; otherwise return empty arrays and warn that full details are at the source. Nutrition numbers must be null unless explicitly supported by source evidence. mealPrepSuitability and protein-rich profile may be inferred, but metadataEvidence must say inferred. Avoid duplicates; return fewer candidates rather than a collection page.`;
